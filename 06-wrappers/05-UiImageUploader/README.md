...
- Изначально компонент может быть как пустым, так и заполненным, если требуется отобразить уже загруженное ранее
  изображение, ещё ДО того, как с компонентом начал работать пользователь. Для этого используется параметр `preview`. Но
  как только пользователь начал работать с компонентом, компонент уже всегда сам определяет своё изображение.
